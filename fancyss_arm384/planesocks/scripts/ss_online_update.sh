...
get_remote_config(){
	...
	#把全部服务器节点写入文件 /usr/share/planesocks/serverconfig/all_onlineservers
...
update_config(){
	#isadded_server=$(uci show planesocks | grep -c "server=\'$server\'")
...
get_v2ray_remote_config(){
	...
	#把全部服务器节点编码后写入文件 /usr/share/planesocks/serverconfig/all_onlineservers
