...
backup_tar(){
	...
	cd /tmp
	mkdir planesocks
	mkdir planesocks/bin
	mkdir planesocks/scripts
	mkdir planesocks/webs
	mkdir planesocks/res
	echo_date "请等待一会儿..."
	TARGET_FOLDER=/tmp/planesocks
	cp /koolshare/scripts/ss_install.sh $TARGET_FOLDER/install.sh
	cp /koolshare/scripts/uninstall_planesocks.sh $TARGET_FOLDER/uninstall.sh
	...
	cp /koolshare/webs/Module_planesocks*.asp $TARGET_FOLDER/webs/
	...
	cp /koolshare/res/icon-planesocks.png $TARGET_FOLDER/res/
	...
	cp /koolshare/res/planesocks.css $TARGET_FOLDER/res/
	cp -r /koolshare/ss $TARGET_FOLDER/
	rm -rf $TARGET_FOLDER/ss/*.json
	tar -czv -f /tmp/planesocks.tar.gz planesocks/
	rm -rf $TARGET_FOLDER
	mv /tmp/planesocks.tar.gz /tmp/files
	echo_date "打包完毕！"
}
