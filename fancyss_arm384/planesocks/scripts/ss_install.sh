...
# planesocks script for koolshare merlin armv7l 384 router with kernel 2.6.36.4
...
rm -rf /koolshare/webs/Module_planesocks*
...
rm -rf /koolshare/res/icon-planesocks.png
...
rm -rf /koolshare/res/planesocks.css
find /koolshare/init.d/ -name "*planesocks.sh" | xargs rm -rf
find /koolshare/init.d/ -name "*socks5.sh" | xargs rm -rf

# 检测储存空间是否足够
echo_date 检测jffs分区剩余空间...
SPACE_AVAL=$(df|grep jffs | awk '{print $4}')
SPACE_NEED=$(du -s /tmp/planesocks | awk '{print $1}')
...
cp -rf /tmp/planesocks/bin/* /koolshare/bin/


echo_date 复制相关的脚本文件！
cp -rf /tmp/planesocks/ss/* /koolshare/ss/
cp -rf /tmp/planesocks/scripts/* /koolshare/scripts/
cp -rf /tmp/planesocks/install.sh /koolshare/scripts/ss_install.sh
cp -rf /tmp/planesocks/uninstall.sh /koolshare/scripts/uninstall_planesocks.sh

echo_date 复制相关的网页文件！
cp -rf /tmp/planesocks/webs/* /koolshare/webs/
cp -rf /tmp/planesocks/res/* /koolshare/res/
...
if [ "`nvram get model`" == "GT-AC5300" ] || [ "`nvram get model`" == "GT-AX11000" ] || [ -n "`nvram get extendno | grep koolshare`" -a "`nvram get productid`" == "RT-AC86U" ];then
	[ -d "/tmp/planesocks/rog/res/" ] && cp -rf /tmp/planesocks/rog/res/* /koolshare/res/
fi
...
dbus set softcenter_module_planesocks_install="4"
dbus set softcenter_module_planesocks_version="$CUR_VERSION"
dbus set softcenter_module_planesocks_title="科学上网"
dbus set softcenter_module_planesocks_description="科学上网"

# 设置v2ray 版本号
dbus set ss_basic_v2ray_version="v4.18.0"

echo_date 一点点清理工作...
rm -rf /tmp/planesocks* >/dev/null 2>&1
