...
PLATFORM=$(cat /koolshare/webs/Module_${MODULE}.asp | tr -d '\r' | grep -Eo "PKG_ARCH=.+"|awk -F "=" '{print $2}'|sed 's/"//g')
PKGTYPE=$(cat /koolshare/webs/Module_${MODULE}.asp | tr -d '\r' | grep -Eo "PKG_TYPE=.+"|awk -F "=" '{print $2}'|sed 's/"//g')
...
update_ss(){
	...
	if [ "${ss_basic_version_local}" != "${fancyss_version_online}" ];then
		...
		fancyss_size_download=$(ls -lh /tmp/${MODULE}.tar.gz |awk '{print $5}')
		fancyss_md5_download=$(md5sum /tmp/${MODULE}.tar.gz | sed 's/ /\n/g'| sed -n 1p)
