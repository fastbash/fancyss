...
update_ss(){
	...
	echo_date "地址：${update_url}/${VERSION}"
	curl -4sk --connect-timeout 10 "${update_url}/${VERSION}" >/tmp/version.json.js
	if [ "$?" != "0" ];then
	...
	fancyss_version_online=$(jq -r '.version' /tmp/version.json.js)
	...
	if [ "${ss_basic_version_local}" != "${fancyss_version_online}" ];then
		...
		fancyss_size_download=$(ls -lh "/tmp/${PACKAGE}.tar.gz" |awk '{print $5}')
		fancyss_md5_download=$(md5sum "/tmp/${PACKAGE}.tar.gz" | sed 's/ /\n/g'| sed -n 1p)
