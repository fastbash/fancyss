...
PLATFORM=$(tr -d '\r' < "/koolshare/webs/Module_${MODULE}.asp" | grep -Eo "PKG_ARCH=.+" | awk -F "=" '{print $2}' | sed 's/"//g')
PKGTYPE=$(tr -d '\r' < "/koolshare/webs/Module_${MODULE}.asp" | grep -Eo "PKG_TYPE=.+" | awk -F "=" '{print $2}' | sed 's/"//g')
...
update_ss(){
	...
	_download "${main_url}/${VERSION}" "/tmp/version.json.js"
	if [ "$?" != "0" ];then
	...
	fancyss_version_online=$(run jq -r '.version' /tmp/version.json.js)
	...
	if [ "${ss_basic_version_local}" != "${fancyss_version_online}" ];then
		...
		cd /tmp
		fancyss_md5_online=$(run jq -r ".$MD5NAME" /tmp/version.json.js)
		...
		_download "${main_url}/${PACKAGE}.tar.gz" "/tmp/${PACKAGE}.tar.gz"
		if [ "$?" != "0" ];then
			echo_date "下载失败！请检查你的网络！"
		fi
		echo_date "${PACKAGE}.tar.gz 下载成功！"
		mv "${PACKAGE}.tar.gz" "${MODULE}.tar.gz"
		fancyss_size_download=$(wc -c < "/tmp/${MODULE}.tar.gz")
		fancyss_md5_download=$(__md5_file "/tmp/${MODULE}.tar.gz")
