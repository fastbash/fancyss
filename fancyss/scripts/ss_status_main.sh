...
failover_action(){
	...
	if [ "$ss_failover_s4_1" = "0" ];then
		[ "$FLAG" = "1" ] && LOGM "${LOGTIME1} fancyss：检测到连续${ss_failover_s1} 个状态故障，关闭插件！"
		[ "$FLAG" = "2" ] && LOGM "${LOGTIME1} fancyss：检测到最近${ss_failover_s2_1} 个状态中，故障次数超过${ss_failover_s2_2}个，关闭插件！"
		[ "$FLAG" = "3" ] && LOGM "${LOGTIME1} fancyss：检测到最近${ss_failover_s3_1} 个状态平均延迟:${PING}超过${ss_failover_s3_2} ms，关闭插件！"
		dbus set ss_basic_enable="0"
		# 关闭
		dbus set ss_heart_beat="1"
		start-stop-daemon -S -q -b -x /koolshare/ss/ssconfig.sh -- stop
	elif [ "$ss_failover_s4_1" = "1" ];then
		[ "$FLAG" = "1" ] && LOGM "${LOGTIME1} fancyss：检测到连续${ss_failover_s1} 个状态故障，重启插件！"
		[ "$FLAG" = "2" ] && LOGM "${LOGTIME1} fancyss：检测到最近${ss_failover_s2_1} 个状态中，故障次数超过${ss_failover_s2_2}个，重启插件！"
		[ "$FLAG" = "3" ] && LOGM "${LOGTIME1} fancyss：检测到最近${ss_failover_s3_1} 个状态平均延迟:${PING}超过${ss_failover_s3_2} ms，重启插件！"
		# 重启
		start-stop-daemon -S -q -b -x /koolshare/ss/ssconfig.sh -- restart
	elif [ "$ss_failover_s4_1" = "2" ];then
		if [ "$ss_failover_s4_2" = "1" ];then
			[ "$FLAG" = "1" ] && LOGM "${LOGTIME1} fancyss：检测到连续${ss_failover_s1} 个状态故障，切换到备用节点：[$(dbus get "ssconf_basic_name_${ss_failover_s4_3}")]！同时把主节点降级为备用节点！"
			[ "$FLAG" = "2" ] && LOGM "${LOGTIME1} fancyss：检测到最近${ss_failover_s2_1} 个状态中，故障次数超过$ss_failover_s2_2 个，切换到备用节点：[$(dbus get "ssconf_basic_name_${ss_failover_s4_3}")]！同时把主节点降级为备用节点！"
			[ "$FLAG" = "3" ] && LOGM "${LOGTIME1} fancyss：检测到最近${ss_failover_s3_1} 个状态平均延迟:$PING 超过$ss_failover_s3_2 ms，切换到备用节点：[$(dbus get "ssconf_basic_name_${ss_failover_s4_3}")]！同时把主节点降级为备用节点！"
			# 切换
			...
			[ "$FLAG" == "1" ] && LOGM "${LOGTIME1} fancyss：检测到连续${ss_failover_s1} 个状态故障，切换到节点列表的下个节点：[$(dbus get "ssconf_basic_name_${NEXT_NODE}")]！"
			[ "$FLAG" == "2" ] && LOGM "${LOGTIME1} fancyss：检测到最近${ss_failover_s2_1} 个状态中，故障次数超过${ss_failover_s2_2} 个，切换到节点列表的下个节点：[$(dbus get "ssconf_basic_name_${NEXT_NODE}")]！"
			[ "$FLAG" == "3" ] && LOGM "${LOGTIME1} fancyss：检测到最近${ss_failover_s3_1} 个状态平均延迟:${PING} 超过${ss_failover_s3_2} ms，切换到节点列表的下个节点：[$(dbus get "ssconf_basic_name_${NEXT_NODE}")]！"
			if [ "$MAXT_NODE" = "1" ];then
...
failover_check_1(){
	...
	local START_MARK="$(sed '/fancyss/d' "$LOGFILE_F" | tail -n "$LINES" | grep "===")"
	if [ -n "$START_MARK" ];then
		#echo "$LOGTIME1 fancyss：1-检测到前$LINES行刚提交，先不检测！"
		return
	fi
	
	local OK_MARK="$(sed '/fancyss/d' "$LOGFILE_F" | tail -n "$ss_failover_s1" | grep -c "200 OK")"
	if [ "$OK_MARK" = "0" ];then
		failover_action 1
	fi
}
...
failover_check_2(){
	...
	local OK_MARK="$(sed '/fancyss/d' "$LOGFILE_F" | tail -n "$ss_failover_s2_1" | grep -vc "200 OK")"
...
failover_check_3(){
	...
	local OK_MARK="$(sed '/fancyss/d' "$LOGFILE_F" | tail -n "$ss_failover_s3_1" | grep -E "200 OK" | grep -oe "⏱ [0-9].* ms" | sed 's/⏱ //g'| sed 's/ ms//g' | awk '{sum+=$1} END {print sum/NR}' | awk '{printf "%.0f\n",$1}')"
...
main(){
	while : ; do
		...
		if ps | grep -v grep | grep -q ssconfig.sh || ps | grep -v grep | grep -q ss_v2ray.sh;then
