...
start_update(){
	...
	# 2. 检测规则本地版本号
	version_gfw_local=$(run jq -r '.gfwlist.date' "${RULE_FILE}" | sed 's/[[:space:]]/_/g')
	version_chn_local=$(run jq -r '.chnroute.date' "${RULE_FILE}" | sed 's/[[:space:]]/_/g')
	version_cdn_local=$(run jq -r '.cdn_china.date' "${RULE_FILE}" | sed 's/[[:space:]]/_/g')
	...
	local rule_save_dir
	rule_save_dir=/koolshare/ss/rules
	local rule_down_dir
	...
	_download "${URL_MAIN}/rules.json.js" "/tmp/rules.json.js"
	if [ "$?" = "0" ]; then
	...
	# 6. 获取在线版本及其它信息
	version_gfw_online=$(run jq -r '.gfwlist.date' /tmp/rules.json.js | sed 's/[[:space:]]/_/g')
	version_chn_online=$(run jq -r '.chnroute.date' /tmp/rules.json.js | sed 's/[[:space:]]/_/g')
	version_cdn_online=$(run jq -r '.cdn_china.date' /tmp/rules.json.js | sed 's/[[:space:]]/_/g')
	
	md5sum_gfw_online=$(run jq -r '.gfwlist.md5' /tmp/rules.json.js)
	md5sum_chn_online=$(run jq -r '.chnroute.md5' /tmp/rules.json.js)
	md5sum_cdn_online=$(run jq -r '.cdn_china.md5' /tmp/rules.json.js)

	count_gfw_online=$(run jq -r '.gfwlist.count' /tmp/rules.json.js)
	count_chn_online=$(run jq -r '.chnroute.count' /tmp/rules.json.js)
	count_ip_chn_online=$(run jq -r '.chnroute.count_ip' /tmp/rules.json.js)
	count_cdn_online=$(run jq -r '.cdn_china.count' /tmp/rules.json.js)
	...
	if [ "${ss_basic_gfwlist_update}" = "1" ];then
		...
		if [ "${version_gfw_local}" != "${version_gfw_online}" ];then
			...
			_download "${URL_MAIN}/gfwlist.conf" /tmp/gfwlist.conf
			...
			if [ "${md5sum_gfwlist_local}" = "${md5sum_gfw_online}" ];then
				...
				local version_gfw_online_tmp
				version_gfw_online_tmp="$(echo "${version_gfw_online}" | sed 's/_/ /g')"
	...
	if [ "${ss_basic_chnroute_update}" = "1" ];then
		...
		if [ "${version_chn_local}" != "${version_chn_online}" ];then
			...
			_download "${URL_MAIN}/chnroute.txt" /tmp/chnroute.txt
			...
			if [ "${md5sum_chnroute_local}" = "${md5sum_chn_online}" ];then
				...
				local version_chn_online_tmp
				version_chn_online_tmp="$(echo "${version_chn_online}" | sed 's/_/ /g')"
	...
	if [ "$ss_basic_cdn_update" = "1" ];then
		...
		if [ "${version_cdn_local}" != "${version_cdn_online}" ];then
			...
			_download "${URL_MAIN}/cdn.txt" /tmp/cdn.txt
			...
			if [ "${md5sum_cdn_local}" = "${md5sum_cdn_online}" ];then
				...
				local version_cdn_online_tmp
				version_cdn_online_tmp="$(echo "${version_cdn_online}" | sed 's/_/ /g')"
	...
	# write number
	nvram set update_ipset="$(run jq -r '.gfwlist.date' "${RULE_FILE}")"
	nvram set update_chnroute="$(run jq -r '.chnroute.date' "${RULE_FILE}")"
	nvram set update_cdn="$(run jq -r '.cdn_china.date' "${RULE_FILE}")"
	
	nvram set ipset_numbers="$(run jq -r '.gfwlist.count' "${RULE_FILE}")"
	nvram set chnroute_numbers="$(run jq -r '.chnroute.count' "${RULE_FILE}")"
	nvram set chnroute_ips="$(run jq -r '.chnroute.count_ip' "${RULE_FILE}")"
	nvram set cdn_numbers="$(run jq -r '.cdn_china.count' "${RULE_FILE}")"
	#======================================================================
	# reboot fancyss
	if [ "${reboot}" = "1" ];then
