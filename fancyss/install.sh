...
platform_test(){
	...
	if [ ! -x "/tmp/${MODULE}/bin/v2ray" ];then
		PKG_TYPE="lite"
	...
	if [ "${PKG_ARCH}" = "arm" ];then
		if [ "${LINUX_VER}" = "26" ];then
			...
			echo_date "下载地址：https://github.com/${github_user}/fancyss_history_package/tree/master/fancyss_hnd"
			exit_install 1
		elif [ "${LINUX_VER}" = "44" ];then
			# RT-AX89X
			echo_date "内核：${KEL_VERS}，架构：${ROT_ARCH}，fancyss_arm_${PKG_TYPE}不适用于该内核版本！"
			echo_date "建议使用fancyss_qca_full或者fancyss_qca_lite！"		
			echo_date "下载地址：https://github.com/${github_user}/fancyss_history_package/tree/master/fancyss_qca"
			exit_install 1
		else
			# future model
	...
	if [ "${PKG_ARCH}" = "hnd" ];then
		if [ "${LINUX_VER}" = "41" ] || [ "${LINUX_VER}" = "419" ];then
			...
			echo_date "下载地址：https://github.com/${github_user}/fancyss_history_package/tree/master/fancyss_arm"
			exit_install 1
...
exit_install(){
	...
	case $state in
		1)
			echo_date "fancyss项目地址：https://github.com/${github_user}/fancyss"
			echo_date "退出安装！"
...
install_now(){
	...
	# 国外dns解析设置为dns2socks
	[ -z "$(dbus get ss_foreign_dns)" ] && dbus set ss_foreign_dns=3
	# others
	...
	[ -z "$(dbus get ss_basic_rule_update)" ] && { 
		...
		cru a ssupdate "0 4 * * * /bin/sh /koolshare/scripts/ss_rule_update.sh"
	}
	# node update 
	[ -z "$(dbus get ss_basic_node_update)" ] && { 
