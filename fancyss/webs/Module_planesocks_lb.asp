...
var dbus = {}
var params = ["ss_lb_passwd", "ss_lb_port", "ss_lb_heartbeat", "ss_lb_up", "ss_lb_down", "ss_lb_interval", "ss_lb_name", "ss_lb_balance", "ss_lb_weight", "ss_lb_mode"];
function init() {
	show_menu(menu_hook);
	get_dbus_data();
}
...
function add_new_lb_node() {
	...
	dbus["ssconf_basic_type_" + cur_lb_node] = db_ss['ssconf_basic_type_' + min_lb_node];
										...
										<div id="lb_setting" style="margin:-25px 0px 0px 0px;">
											<table style="margin:-1px 0px 0px 0px;" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable" id="routing_table">
												...
												<tr>
													<th>负载均衡模式</th>
													<td>
														<select id="ss_lb_balance" name="ss_lb_balance" style="width:100px;margin:0px 0px 0px 2px;" class="input_option" onchange="update_visibility();">
															<option value="source" selected>源IP</option>
															<option value="roundrobin">轮询</option>
															<option value="weight" >权重</option>
															<option value="leastconn" >最少连接数</option>
