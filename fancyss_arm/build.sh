...
sync_v2ray_binary(){
	...
	if [ "$md5_latest"x != "$md5_old"x ]; then
		...
		cp -rf ../v2ray_binary/$v2ray_version/v2ray planesocks/bin/
...
do_build() {
	...
	"md5":"$md5value",
	"name":"$MODULE",
	"tar_url": "https://raw.githubusercontent.com/fastbash/fancyss/master/fancyss_arm/planesocks.tar.gz",
	"title":"$TITLE",
	"version":"$VERSION"
	}
	EOF
